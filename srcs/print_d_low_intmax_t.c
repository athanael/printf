...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/29 14:11:43 by dfouquet          #+#    #+#             */
/*   Updated: 2017/09/29 15:31:27 by dfouquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_printf.h"

int		preci_d_low_intmax_t(int *str, intmax_t arg)
{
	intmax_t	nbr;
	int			len;
	int			bn;
...
int		print_d_low_intmax_t_ter(int *str, intmax_t arg, int len, int bn)
{
	...
	if (str[4] == '0' && str[2] == -1)
		...
		bn += preci_d_low_intmax_t(str, arg);
	if (str[5] == '-')
	{
		len = str[1] - bn;
		while (len-- > 0 && ++bn)
...
int		print_d_low_intmax_t_bis(int *str, intmax_t arg, int len, int bn)
{
	...
	return (print_d_low_intmax_t_ter(str, arg, len, bn));
}
...
int		print_d_low_intmax_t(va_list ap, int *str)
{
	...
	if (str[6] == '+' && arg >= 0)
		--len;
	return (print_d_low_intmax_t_bis(str, arg, len, bn));
}
